...
const PrescriptionRefillChat = ({ onExit, onComplete }: Props) => {
  ...
  // Auto-open the camera/file picker only when the upload path first lands on step 2.
  // For retake/re-upload, we manually open the picker and skip this auto-open once
  // to prevent the picker from opening twice.
  useEffect(() => {
    if (step === 2 && sub2 === "upload-pending") {
  ...
  const openUploadPickerManually = () => {
    if (step === 2 && sub2 !== "upload-pending") {
      ...
      pushMsg({
        ...
        node: (
          <div className="space-y-2">
            ...
            <div className="flex gap-2 flex-wrap">
              <button
                onClick={openUploadPickerManually}
                className="rounded-full border border-border bg-background text-foreground font-medium hover:bg-accent transition-colors px-4"
                style={{ minHeight: 40, fontSize: 14 }}
              >
                {lastCaptureMethod === "upload"
