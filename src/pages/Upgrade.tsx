...
function planMatchesSubscription(plan: Plan, sub: { plan_name?: string; plan_key?: string; plan_id?: string } | null): boolean {
...
const Upgrade = () => {
  ...
  const applyCurrentPlan = useCallback((sub: { plan_name?: string; plan_key?: string; plan_id?: string } | null) => {
    const label = sub?.plan_name || (sub?.plan_key === "pro" ? "Pro" : sub?.plan_key === "enterprise" ? "Enterprise" : "Basic");
    setCurrentPlanLabel(label);
    setPlans((prev) =>
      prev.map((p) => ({
        ...p,
        current: planMatchesSubscription(p, sub),
      }))
    );
  }, []);
          ...
          setPlans((prev) => prev.map((p) => {
            const apiPlan = data.find(
              (ap: { id?: string; plan_id?: string; name?: string }) =>
                ap.id === p.id ||
                ap.plan_id === p.id ||
                normalizePlanKey(ap.name) === normalizePlanKey(p.name)
            );
            if (apiPlan) {
              ...
              dynamicFeatures.push(
                isScansUnlimited
                  ...
                  : `${scans} Face Scan${String(scans) !== "1" ? "s" : ""} / month`
              );
              dynamicFeatures.push(
                isCreditsUnlimited
                  ? "Unlimited Chat Credits"
                  : `${Number(credits).toLocaleString()} Chat Credits`
              );
              try {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-12">
          ...
          {!loadingPlan && (
            <p className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary text-sm font-medium">
            ...
            return (
              <div
                ...
                className={`relative bg-card/80 backdrop-blur-sm border rounded-2xl p-6 shadow-sm hover:shadow-lg transition-all ${
                  plan.current ? "border-emerald-500/50 ring-2 ring-emerald-500/20" : (plan as Plan & { popular?: boolean }).popular ? "border-primary/40 ring-2 ring-primary/10" : "border-border/50"
                }`}
              >
                {plan.current && (
                ...
                {(plan as Plan & { popular?: boolean }).popular && !plan.current && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                ...
                <button
                  ...
                  className={`w-full h-11 rounded-xl text-sm font-medium transition-all flex items-center justify-center gap-2 ${plan.current ? "bg-secondary text-muted-foreground cursor-default" : (plan as Plan & { popular?: boolean }).popular ? "bg-gradient-to-r from-primary to-primary/80 text-primary-foreground shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30" : "border border-border/60 text-foreground hover:bg-accent"} disabled:opacity-60`}
                >
