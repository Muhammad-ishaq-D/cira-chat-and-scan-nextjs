...
import { ArrowLeft, Check, CreditCard, Shield, Zap, Crown, Sparkles, Star } from "lucide-react";
...
const plans = [
  {
    ...
    price: "Free",
    period: "",
    desc: "Get started with essential health insights",
    icon: Shield,
        ...
        <div className="relative z-10 max-w-lg mx-auto px-6 py-12">
          ...
          <div className="bg-card/80 backdrop-blur-sm border border-border/50 rounded-2xl p-6 mb-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-lg font-semibold text-foreground" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
                  {plan?.name} Plan
                </h2>
                <p className="text-sm text-muted-foreground">{plan?.desc}</p>
              </div>
              <div className="text-right">
                <p className="text-2xl font-bold text-foreground" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
                  {plan?.price}
                </p>
          ...
          <form onSubmit={handlePayment} className="bg-card/80 backdrop-blur-sm border border-border/50 rounded-2xl p-6 shadow-sm space-y-5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-sm font-semibold text-foreground" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>Payment Details</h3>
            </div>
            ...
            <div>
              ...
              <input
                ...
                required
                className="w-full h-11 rounded-xl border border-border/60 bg-background/50 px-4 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 transition-all"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                required
                className="w-full h-11 rounded-xl border border-border/60 bg-background/50 px-4 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 transition-all font-mono tracking-wider"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="MM/YY"
                  required
                  className="w-full h-11 rounded-xl border border-border/60 bg-background/50 px-4 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 transition-all font-mono"
                />
              </div>
              <div>
            ...
            <button
              type="submit"
              disabled={processing}
              className="w-full h-12 rounded-xl bg-gradient-to-r from-primary to-primary/80 text-primary-foreground font-medium text-sm shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 transition-all disabled:opacity-60"
            >
              {processing ? (
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {plans.map((plan) => {
            ...
            return (
              <div
                key={plan.id}
                className={`relative bg-card/80 backdrop-blur-sm border rounded-2xl p-6 shadow-sm hover:shadow-lg transition-all ${
                  plan.popular ? "border-primary/40 ring-2 ring-primary/10" : "border-border/50"
                }`}
              >
                {plan.popular && (
