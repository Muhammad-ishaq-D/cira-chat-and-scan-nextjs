...
const VitalsScan = () => {
  ...
  return (
    <div className="flex bg-background" style={{ height: '100dvh' }}>
      ...
      {/* ═══════════ CAMERA VIEW — exact desktop layout ═══════════ */}
      {/* Always kept in DOM so the ShenAI SDK can finish its deferred DOM cleanup
          without hitting unmounted nodes (removeChild NotFoundError). */}
      <div
      ...
      >
        {/* ── Left: Instructions Panel ── */}
        <aside className="hidden lg:flex w-[22rem] xl:w-[24rem] 2xl:w-[25rem] shrink-0 bg-white border-r border-gray-100 flex-col overflow-y-auto">
          <div className="px-6 py-6">
            <div className="flex items-center gap-3 mb-8">
              <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center shrink-0">
                <ScanFace size={18} className="text-primary" />
              </div>
              <h2 className="font-heading font-semibold text-[20px] leading-none text-foreground">How to Scan</h2>
            </div>
            ...
            <div className="space-y-9">
              {[
                ...
                <div key={item.step} className="flex gap-4">
                  ...
                  <div>
                    <p className="text-base font-semibold text-foreground font-heading leading-tight">{item.title}</p>
                    <p className="text-sm text-muted-foreground font-body mt-2 leading-relaxed max-w-[260px]">{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>

            <div className="mt-9 p-4 rounded-2xl bg-gray-50 border border-gray-100 shadow-sm">
              <div className="flex items-center gap-2 mb-2">
                <ShieldCheck size={15} className="text-emerald-500" />
                <p className="text-sm font-bold text-foreground">100% Private</p>
              </div>
              <p className="text-xs text-muted-foreground leading-relaxed">All analysis happens on your device. No video is recorded or transmitted.</p>
            </div>
            ...
            {!isGuest && (
              <div className="mt-4 p-4 rounded-2xl bg-primary/5 border border-primary/10">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-sm font-bold text-primary">Scan Credits</p>
                </div>
            ...
            {!isGuest && (
              <button
                onClick={() => { const next = !showHistory; setShowHistory(next); if (next) logAuditEvent("OPEN_VITALS_SCAN_HISTORY"); }}
                className="mt-5 w-full h-12 rounded-2xl border border-border/70 bg-white text-base font-medium text-muted-foreground hover:bg-accent hover:text-foreground transition-all flex items-center justify-center gap-3"
              >
        ...
        {/* ── Center: Camera + bottom scan controls ── */}
        <main className="relative flex-1 min-w-0 bg-white overflow-hidden">
          <button
            onClick={() => navigate("/dashboard")}
            className="hidden lg:flex absolute top-5 right-6 z-30 w-12 h-12 rounded-2xl items-center justify-center text-white bg-gray-300 hover:bg-gray-400 transition-all shadow-sm"
          ...
          <div className="h-full flex flex-col items-center">
            <div className="relative w-full flex justify-center">
              <div className="relative w-[410px] h-[614px] max-w-[calc(100vw-2rem)] max-h-[calc(100dvh-210px)] bg-black overflow-hidden">
                <canvas
                  id={CANVAS_ID}
                  ref={canvasRef}
                  className="absolute inset-0 w-full h-full object-cover"
            ...
            <div className="w-full px-2 mt-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 h-[96px]">
                <div className="relative overflow-hidden rounded-lg border border-gray-200 bg-white">
                  <p className="absolute top-3 left-3 text-base font-medium text-gray-800">PULSE</p>
                  <Heart size={78} strokeWidth={1.5} className="absolute -right-2 top-3 text-gray-300" />
                </div>
                <div className="relative overflow-hidden rounded-lg border border-gray-200 bg-white">
                  <p className="absolute top-3 left-3 text-base font-medium text-gray-800">BLOOD PRESSURE</p>
                  <Activity size={86} strokeWidth={1.5} className="absolute -right-3 top-4 text-gray-300" />
                </div>
              </div>

              <div className="relative mt-1 h-[96px] rounded-lg border border-gray-200 bg-white">
                  ...
                  {status === "ready" && (
                    noScansLeft ? (
                      <button
                        type="button"
                        onClick={() => navigate("/upgrade")}
                        className="relative z-30 px-7 h-16 rounded-full bg-gradient-to-br from-amber-500 to-orange-500 text-white shadow-2xl shadow-amber-400/40 hover:shadow-amber-400/60 transition-all flex items-center justify-center gap-3 active:scale-95 ring-4 ring-amber-400/20 font-semibold text-base"
                        style={{ pointerEvents: "auto" }}
                      >
                        <Crown size={22} />
                        <span>Upgrade to Scan</span>
                      ...
                      <button
                        ...
                        className="relative z-30 px-8 h-16 rounded-full bg-gradient-to-br from-primary to-primary/80 text-primary-foreground shadow-2xl shadow-primary/40 hover:shadow-primary/60 transition-all flex items-center justify-center gap-3 active:scale-95 ring-4 ring-primary/20 font-semibold text-base"
                        style={{ pointerEvents: "auto" }}
                      >
                        <Heart size={25} />
                        <span>Start</span>
        ...
        {/* ── Right: Vitals Panel ── */}
        <aside className="hidden lg:flex w-[22rem] xl:w-[24rem] 2xl:w-[25rem] shrink-0 bg-white border-l border-gray-100 flex-col overflow-y-auto">
          <div className="px-8 py-6">
            <div className="flex items-center gap-3 mb-8">
              <div className="w-12 h-12 rounded-2xl bg-rose-50 flex items-center justify-center shrink-0">
                <Heart size={18} className="text-rose-500" />
              </div>
              <h2 className="font-heading font-semibold text-[20px] leading-none text-foreground">Vitals</h2>
            </div>

            <div className={`mb-5 px-5 h-[54px] rounded-2xl flex items-center gap-3 ${
              (status === "measuring" || status === "processing") ? "bg-primary/10 border border-primary/20" :
              status === "ready" ? "bg-emerald-50 border border-emerald-200" :
              "bg-gray-50 border border-gray-100"
            }`}>
              {(status === "measuring" || status === "processing") ? (
                ...
                <>
                  ...
                  <p className="text-sm font-semibold text-emerald-700">Ready to scan</p>
                </>
              ) : (
                <>
            ...
            <div className="space-y-3">
              {[
                ...
                const Icon = vital.icon;
                return (
                  <div key={vital.label} className="flex items-center gap-4 h-[81px] px-4 rounded-2xl bg-gray-50/80 border border-gray-100">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${vital.bg}`}>
                      <Icon size={17} className={vital.color} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-muted-foreground font-body">{vital.label}</p>
                      <p className="text-base font-semibold text-foreground font-heading leading-tight">
                        --<span className="text-[11px] text-muted-foreground font-normal ml-1">{vital.unit}</span>
                      </p>
                    </div>
                    {(status === "measuring" || status === "processing") && (
                      <div className="w-1.5 h-1.5 rounded-full bg-primary/40 animate-pulse shrink-0" />
