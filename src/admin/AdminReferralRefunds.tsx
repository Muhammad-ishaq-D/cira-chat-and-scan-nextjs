...
const AdminReferralRefunds = () => {
  const [refunds, setRefunds] = useState<ReferralRefundRequest[]>([]);
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Failed to load referral refund requests");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDecide = async (refund: ReferralRefundRequest, decision: "approve" | "reject") => {
    ...
    try {
      ...
      showToast(
        refund.id,
        decision === "approve" ? "Refund approved and issued." : "Refund rejected.",
        decision === "approve"
      );
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8 pb-24 md:pb-8 space-y-6">
      ...
      {!error && refunds.length === 0 && (
        <div className="bg-card/80 backdrop-blur-sm border border-border/50 rounded-xl p-12 flex flex-col items-center gap-3 text-center">
          ...
          <p className="font-medium text-foreground">No pending referral refund requests</p>
          <p className="text-sm text-muted-foreground">All referral refund requests have been reviewed.</p>
        </div>
      )}
...
export default AdminReferralRefunds;
